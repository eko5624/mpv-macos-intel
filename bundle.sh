...
first_libdeps=($(get_deps $(otool -L $DIR/mpv/build/mpv.app/Contents/MacOS/mpv | grep -e '\t' | grep -Ev "\/usr\/lib|\/System|@rpath" | awk 'NR==1 { print $1 }' | xargs basename | grep -Ev "libluajit" | sed 's#^#/usr/local/lib/#') | sort -u))
...
for f in "${all_deps[@]}"; do
  ...
  elif [[ "$(basename $f)" = "libluajit"* ]]; then
    sudo cp `brew --prefix luajit`/lib/$(basename $f) $DIR/mpv/build/mpv.app/Contents/MacOS/lib
  else
