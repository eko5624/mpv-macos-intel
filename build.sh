...
if build "openssl" "$ver_openssl@1.1"; then
  download "https://www.openssl.org/source/openssl-$ver_openssl@1.1.tar.gz"
  if $MACOS_M1; then
  ...
  build_done "openssl" "$ver_openssl@1.1"
fi
...
if build "python" "$ver_python@3.11"; then
  cd $PACKAGES
  git clone https://github.com/python/cpython --branch "$ver_python@3.11"
  cd cpython
  execute ./configure \
    --prefix="${WORKSPACE}" \
    --with-pydebug \
  ...
  build_done "python" "$ver_python@3.11"
fi
